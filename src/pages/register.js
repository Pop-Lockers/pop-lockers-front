...
async componentDidMount() {
    ...
    renderTeamOptions = () => {
        return this.state.teams.map((team) => {
            return (
                <option key={team.id} data-id={team.id} value={team.id}>{team.business_name}, {team.location}</option>
            )
        })
    }
            ...
            <Layout>
                <Form className={registerStyles.form} onSubmit={this.handleSubmitForm}>
                    ...
                    <FormGroup>
                        ...
                        <Input onChange={this.handleSelectInputChange} name="team_id"  type="select">
                            <option defaultValue='select an option'>Pick Team from Dropdown</option>
